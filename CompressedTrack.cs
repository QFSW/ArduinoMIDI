...
public class CompressedTrack
{
    ...
    public static CompressedTrack Build(Track track)
    {
        ...
        foreach ((long semitone, int index) in semitoneTable)
        {
            int frequency = 
                semitone == -1 
                ? 0
                : (int)(440 * Math.Pow(2.0, (semitone - 69) / 12.0));
        ...
        {
            int durationMs = TimeConverter.ConvertTo<MetricTimeSpan>(duration, track.tempoMap).Milliseconds;
            compressedTrack.durationMsTable[index] = (int)durationMs;
        }
    ...
    public void GenerateHeaderCode(string path)
    {
        ...
        int flashUsageBytes =
            semitoneFrequencyTable.Length * 2   +
            durationMsTable.Length * 2          +
            compressedSemitones.Length          +
...
// Uses {flashUsageBytes:n0} bytes of flash
// Compressed track header
...
// Mapping tables
const PROGMEM uint16_t semitoneFrequencyTable[NUM_UNIQUE_SEMITONES] = {{{semitoneTableSerialized}}};
const PROGMEM uint16_t durationMsTable[NUM_UNIQUE_DURATIONS] = {{{durationTableSerialized}}};
